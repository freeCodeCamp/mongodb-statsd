...
function createGraphiteTransform(server, metricMap, lastMetrics) {
  ...
  return serverStatus => {
    log('received metrics', serverStatus);
    const metrics = {
      keyValues: {},
      counters: {},
    };
    ...
    function addMetric(metrics, metricType, name, value) {
      ...
      const metricKey =
        metricType === 'counter' ?
          _.template(config.graphiteKeyTemplateCounters, templateData) :
          _.template(config.graphiteKeyTemplateGauges, templateData);
